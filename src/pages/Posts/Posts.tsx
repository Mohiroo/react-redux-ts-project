...
const Posts = () => {
  ...
  const { data: serverPosts, error, isLoading } = PostAPI.useGetPostsQuery(postsLimit);
  ...
  return (
    <div>
      ...
      {!error && <div ref={lastElement} className={styles.observer}></div>}
      {!error && isLoading && <h1  className={styles.posts__title}>Загружаю посты...</h1>}
    </div>
  );
};
